...
function calculateStreak(dates: string[]): number {
  ...
  const uniqueDates = [...new Set(dates.map((d) => d.split("T")[0]))].sort((a, b) =>
    b.localeCompare(a)
  );
...
export const getInsights = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const moodByDate = new Map<string, { mood: number; count: number }>();
    const moodByHour = new Map<number, { mood: number; count: number }>();
    const energyByDow = new Map<number, { energy: number; count: number }>();
    const tagStatsMap = new Map<string, { mood: number; count: number }>();
    ...
    let peakHour: { hour: string; avgMood: number } | null = null;
    ...
    const avgRecentEnergy =
      recent3.length > 0 ? recent3.reduce((acc, e) => acc + e.energy, 0) / recent3.length : 0;
